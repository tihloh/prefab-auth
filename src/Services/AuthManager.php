...
final class AuthManager
{
    ...
    public function prefabConfigure(): void
    {
        if (!$this->session) {
            ...
            if ($session['value'] instanceof AuthSessionStoreInterface) {
                ...
                $sessionKey = PrefabConfig::resolve('auth', 'session_key', $this->config, 'prefab_auth_user');
                $this->session = new NativeSessionStore((string) $sessionKey['value']);
                PrefabRuntime::recordResolution('auth', 'session', $sessionKey['source'], ['provider' => NativeSessionStore::class, 'session_key' => (string) $sessionKey['value']]);
            }
        }
        ...
        if (!$this->users) {
            ...
            if ($provider['value'] instanceof AuthUserProviderInterface) {
                ...
                if ($entry && $prefabUsers && $entry['provider'] === 'prefab-users') {
                    ...
                    PrefabRuntime::recordResolution('auth', 'user_provider', 'prefab-capability', ['provider' => $entry['provider'], 'adapter' => PrefabUsersAuthProvider::class]);
        ...
        if (!$this->credentials) {
            $configured = PrefabConfig::resolve('auth', 'credential_store', $this->config);
            if ($configured['value'] instanceof AuthCredentialStoreInterface) {
                $this->credentials = $configured['value'];
                PrefabRuntime::recordResolution('auth', 'credential_store', $configured['source'], ['provider' => $this->credentials::class]);
            } else {
                $pdo = $this->credentialDatabase();
                if ($pdo) {
                    $table = PrefabConfig::resolve('auth', 'credentials_table', $this->config, 'prefab_auth_credentials');
                    $this->credentials = new PdoCredentialStore($pdo, (string) $table['value']);
                    PrefabRuntime::recordResolution('auth', 'credential_store', 'database-store', ['provider' => PdoCredentialStore::class, 'table' => (string) $table['value']]);
                }
            }
        }

        if ($this->credentials) {
            PrefabRuntime::provide('auth_credentials', $this->credentials, 'prefab-auth');
        }
    ...
    public function explain(): array { return PrefabRuntime::explain('auth'); }
    ...
    {
        if ($password === '') throw new RuntimeException('Password cannot be empty.');
        if (!$this->provider()->findById($userId)) throw new RuntimeException('Cannot set a password for an unknown user.');
        $this->credentialStore()->setPasswordHash($userId, password_hash($password, PASSWORD_DEFAULT));
    }

    public function hasPassword(int|string $userId): bool { return $this->passwordHashForId($userId) !== null; }
    ...
    {
        if (!$this->verifyPassword($userId, $currentPassword)) return false;
        $this->setPassword($userId, $newPassword);
        return true;
    }

    public function removePassword(int|string $userId): void { $this->credentialStore()->remove($userId); }
    ...
    public function attempt(string $identifier, string $password, array $context = []): AuthResult
    {
        ...
        if (!$user || !$user->authIsActive()) {
            return $this->result(false, null, $this->log('auth.login_failed', null, $context, ['identifier' => $identifier]), 'invalid_credentials');
        }

        $hash = $this->passwordHashFor($user);
        if (!$hash || !password_verify($password, $hash)) {
            return $this->result(false, null, $this->log('auth.login_failed', $user->authId(), $context), 'invalid_credentials');
        }
    ...
    {
        if (!$this->credentials) $this->prefabConfigure();
        return $this->credentials ?? throw new RuntimeException('Prefab Auth needs a credential store or database to manage passwords.');
    }

    private function passwordHashFor(AuthenticatableUserInterface $user): ?string
    {
        if (!$this->credentials) $this->prefabConfigure();
        return $this->credentials?->passwordHash($user->authId()) ?: $user->authPasswordHash();
    }
    ...
    private function credentialDatabase(): ?PDO
    {
        foreach ([
            $this->config['database'] ?? null,
            PrefabConfig::moduleOnly('auth')['database'] ?? null,
            PrefabConfig::get('database'),
            PrefabRuntime::resolveEntry('database')['value'] ?? null,
        ] as $value) {
            if ($value instanceof PDO) return $value;
            if (is_object($value) && method_exists($value, 'pdo')) {
                $pdo = $value->pdo();
